/**
 * @fileOverview Constraint Solver <p>{@link hotdrink.model.graph.Solver}</p>
 * @author Wonseok Kim
 */

//provides("hotdrink.model.graph.Solver");
...
(function () {
  ...
  var Constraint = function (c, cc, strength) {
    this.name = c;
    this.outer = cc;
    /* For me: holds references. */
    this.variables = [];      // Array<Variable>
    /* For clients: holds names. */
  ...
  var Solver = function (graph) {
    ...
    Object.keys(this.g.variables).forEach(function (v) {
      var vv = this.g.variables[v];
      vv.writtenBy = "";
      this.variables[v] = new Variable(v, vv);
    ...
    = function eliminateConstraint(ccc, mNext)
  {
     ...
     * search. */
    if (mPrev) {
      var mmPrev = this.g.methods[mPrev];
      mmPrev.isSelected = false;
      mmPrev.outputs.forEach(function (w) {
        this.variables[w].determinedBy = null;
        this.g.variables[w].writtenBy = "";
      }, this);
    }

    if (mNext) {
      var mmNext = this.g.methods[mNext];
      mmNext.isSelected = true;
      mmNext.outputs.forEach(function (w) {
        this.variables[w].determinedBy = ccc;
        if (ccc.isRequired()) {
          this.g.variables[w].writtenBy = mNext;
        }
      }, this);
    }
  ...
  Solver.prototype.undo = function undo() {
    while (this.undoStack.length > 0) {
      ...
      if (mPrev) {
        var mmPrev = this.g.methods[mPrev];
        mmPrev.isSelected = true;
        mmPrev.outputs.forEach(function (w) {
          this.variables[w].determinedBy = ccc;
          if (ccc.isRequired()) {
            this.g.variables[w].writtenBy = mPrev;
          }
        }, this);
      }
  ...
  namespace.open("hotdrink.model.graph").Solver = Solver;
