...
(function () {
   ...
   */
  var blame = function blame(model, v) {
   ...
   * @param {String} variable
   */
  var poison = function poison(model, v) {
    /* If this variable has already been visited, then we can go home early. */
    var vv = model.variables[v];
    if (vv.isPoisoned) {
      return;
   ...
   */
  var doCommandActivation = function doCommandActivation(model) {
   ...
   * @param {String} variable
   */
  var markRelevant = function markRelevant(model, v) {
    /* If this variable has already been visited, then we can go home early. */
    var vv = model.variables[v];
    if (vv.isRelevant) {
      return;
   ...
   * @param {String} variable
   */
  var maybeMarkCanBeRelevant = function maybeMarkCanBeRelevant (model, v) {
    var mark = canBeRelevant(model, v, /*visited=*/{});
   ...
   * @param {hotdrink.Model} model
   * @param {String} v
   *   Reachable variable to consider.
   * @param {Object} visited
   *   Variables marked as visited.
   */
  var canBeRelevant = function canBeRelevant(model, v, visited) {
    ...
    var vv = model.variables[v];
    if (vv.isRelevant || vv.canBeRelevant) {
    ...
    var determinedBy = model.methods[m].constraint;
    var ms = model.constraints[determinedBy].methods;
    ...
    for (var i = 0; i < ms.length; ++i) {
      ...
      for (var j = 0; j < ws.length; ++j) {
        var w = ws[j];
        /* TODO: Would a simple check for self-loops (w == v) here before
         * recursion improve performance? */
        if (canBeRelevant(model, w, visited)) {
          return true;
        }
      }
    }
   ...
   * @param {hotdrink.Model} model
   */
  var doWidgetEnablement = function doWidgetEnablement(model) {
   ...
   *   </pre>
   */
  EnablementGraph.initialize = function initialize() {
   ...
   */
  EnablementGraph.update = function update(events) {
    Object.keys(this.variables).forEach(function (v) {
  ...
  /**
   * @name hotdrink.model.behavior
   * @namespace Pluggable behaviors.
   */
  namespace.open("hotdrink.model.behavior").Enablement = EnablementGraph;
