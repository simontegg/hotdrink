...
(function () {
  ...
  var maybeMarkCanBeRelevant = function (model, ngraph, v) {
    ngraph.variables.get(v).canBeRelevant
      = canBeRelevant(model, ngraph, /*w=*/v, /*visited=*/{});
    DEBUG_BEGIN;
  ...
  var canBeRelevant = function (model, ngraph, w, visited) {
    ...
    model.cgraph.variables.get(w).usedBy.each(function (m) {
      /* Only find descendants through methods that are not in the current
       * solution graph. */
      if (model.sgraph.methods.get(m).isSelected) {
        return;
      }
      model.cgraph.methods.get(m).outputs.each(function (u) {
