...
(function () {
  ...
  /**
   * Search the ancestor tree of v in the solution graph, until we can
   * determine whether or not v can be relevant to an output variable after
   * editing it.
   ...
   * @param {String} v
   * @param {String} w
   *   Ancestor to consider.
   * @param {Object} visited
   *   Variables marked as visited.
   */
  var maybeMarkCanBeRelevant1 = function (model, ngraph, v, w, visited) {
    ...
    /* Test: if v can reach a relevant variable, then it can be relevant. */
    ...
    if (m !== "") {
      model.cgraph.methods.get(m).inputs.each(function (u) {
        ...
        maybeMarkCanBeRelevant1(model, ngraph, v, u, visited);
      });
    }
  };
