...
(function () {
  ...
  var sheetDecl = action(
      wseq(expect("sheet"), identifier, expect("{"), many(sectionDecl, true), expect("}")),
      function (ast) {
        var id = ast[0];
  ...
  var Variable = function (cellType, initializer) {
    this.cell_type = cellType ? cellType : null;
    this.used_by = [];
    if (initializer) {
    ...
    for (var i=0; i<cells.length; ++i) {
      ...
      if (cell.type === Type.VAR_DECL) {
          ...
          if (v == null) {
            if (useExistingVariable) {
              ...
              if (expr !== null && useInitializer) {
                initializer = transformInitializer(expr);
              }
  
              v = new Variable(sectionType, initializer);
        ...
        if (!useInitializer) {
          ...
          var used = t.used;
          /* update used_by and check variables */
          for (var k=0; k<used.length; ++k) {
            ...
            if (usedv == null) {
              ...
              if (!contains(usedv.used_by, methodId))
                usedv.used_by.push(methodId);
            }
          }
   ...
   * which replaces var_ref with values.get(id) */
  ...
  var MethodWriter = Class.create(ExprParser.DefaultCodeVisitor, {
    ...
    visitVarRef : function (node) {
      ...
      /* if this variable is one of outputs, it doesn't belong to inputs
       * and should be evaluated with 'E.last_eval1' function. 
       */
      var code;
      if (contains(this.outputs, vid)) {
        code = "E.last_eval1(\"" + vid + "\")";
        /* do not put into inputs */
      } else {
        code = "E.eval1(\"" + vid + "\")";
  ...
  var transformMethod = function (graph, outputs, expr) {
    ...
    var functionDef = "function(E) {return (" + code + ");}";
  ...
  var parse = function (input) {
    ...
    if (reply.state.length > 0) {
      raid.warning("Invalid string after sheet - " + reply.state.toString());
    }
    
    var sheet = reply.ast;
    raid.debug("parsed sheet\n" + sheetToString(sheet));
    
    var graph = constructGraph(sheet);
    raid.debug("parser result\n" + graphToString(graph));
  ...
  /* Internal: debug printout of sheet */
