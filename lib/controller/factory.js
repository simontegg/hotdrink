...
(function () {
  ...
  /** @lends hotdrink.controller.Factory# */
  {
     ...
     *
     * <p>
     * For documentation on the tree type (ViewAst) that should be accepted by
     * find and build visitors, see
     * {@link hotdrink.controller.Factory#buildAndBind}.
     * </p>
     *
     ...
     * @returns {Element[]}
     *   A tree of HTML elements built from the AST.
     */
    buildAndBind : function (tree, model) {},
    /**
     ...
     * @param {hotdrink.controller.ModelController} model
     */
    findAndBind : function (tree, model) {
      var factory = this;
      tree.each(function (item) {
        if (item.children) {
          factory.findAndBind(item.children);
        }
        var helpers = factory.widgets.get(item.type);
        var view = helpers.find(item);
        helpers.binders.each(function (bind) {
          bind(model, view, item.options);
        });
      });
    }
  });
