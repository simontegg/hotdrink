...
(function () {
  ...
  /** @lends hotdrink.controller.Factory# */
  {
     ...
     * model.
     * @param {[Object:ViewAst]} trees
     *   <p>See {@link hotdrink.controller.Factory#compile}</p>
    ...
    buildAndBind : function (trees, model) {
      ...
      this.buildAndBind1(root);
      return root[0].element.box;
    },
    buildAndBind1 : function (trees, model) {
      ...
      trees.each(function (tree) {
        ...
        var view = tree.view;
        if (!view) {
          ERROR("could not buid view for " + tree.type);
          /* TODO: Break out of the whole function. This is a fatal error. */
          return;
        }
        /* Bind the view. */
    ...
    /**
     * Process the AST, searching by id for existing views and
     * binding them to the model.
     * @param {[Object:ViewAst]} trees
     *   <p>See {@link hotdrink.controller.Factory#compile}</p>
     *   <p>Note: only nodes with an id corresponding to an element
     *   in the DOM will be bound.</p>
     * @param {hotdrink.controller.ModelController} model
     */
    findAndBind : function (trees, model) {
      ...
      trees.each(function (tree) {
        /* Handle children first. */
        if (tree.children) {
          factory.findAndBind(tree.children, model);
        }
        /* Find the view. */
        if (!tree.options.id) {
          LOG("unidentified " + tree.type);
          return;
        }
        var helpers = factory.widgets.get(tree.type);
        if (!(helpers && helpers.find)) {
          WARNING("unsupported widget type: " + tree.type);
          return;
        }
        var view = helpers.find(tree);
        if (!view) {
          WARNING("could not find view for #" + tree.options.id);
          return;
        }
        /* Bind the view. */
        LOG("binding #" + view.identify());
        helpers.binders.each(function (bind) {
          bind(model, view, tree.options);
        });
      });
