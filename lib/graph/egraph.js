...
(function () {
  ...
  /** @lends hotdrink.graph.EGraph# */
  {
    ...
    update : function (cgraph, sgraph, changeSet) {
      ...
      /* Inputs to new methods in the solution need to be marked relevant for
       * the methods to be executed at least once to find whether the inputs are
       * actually relevant. */
      sgraph.planDiff.each(function (m) {
        egraph.methods.get(m).inputs.each(function (pair) {
          pair.value.isRelevant = true;
        });
      });
