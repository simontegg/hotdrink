...
(function () {
  ...
  var IGraph = Class.create({
    initialize : function (/*CGraph*/ cgraph) {
      ...
      Object.keys(cgraph.variables).forEach(function (id){
        _variables.set(id, new Variable(id));
      });
      Object.keys(cgraph.methods).forEach(function (m){
        var mm = cgraph.methods[m];
        
        var mt = new Method(m);

        mm.inputs.forEach(function (v) {
          mt.inputs.push(_variables.get(v));
        });
        mm.outputs.forEach(function (v) {
          mt.outputs.push(_variables.get(v));
        });
        _methods.set(m, mt);
      });
      Object.keys(cgraph.constraints).forEach(function (c) {
        var method_ids = cgraph.constraints[c].methods;
        ...
        var cn = new Constraint(c, Strength.REQUIRED, variables, methods);

        _constraints.set(c, cn);
      });
