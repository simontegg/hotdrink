...
(function () {
  ...
  var log = function (msg) {
    if(solve_debug) {
      writeDebug("[solve] " + msg);
    }
  };
  
  var logObject = function (obj) {
    if(solve_debug) {
      writeDebugObject(obj);
    }
  };
   ...
   * @return {Array} the sorted list of vertices.
   */
   ...
   * @param {Array<Constraint>} cnsQueue
   * @return {Constraint} the removed element.
   */
   ...
   * @param {Array<Method>} selectedMethods
   * @return {Array<String>} sorted method ids.
   */
  ...
  /**
   * Solve the given constraint graph with variable priority.
   * 
   * @param {Array<String>} priority  array of variable names in decreasing order of strength.
   * @param {boolean} sort  (optional, default is false) if true the result would be sorted topologically.
   * @return {Array<String>} array of selected method names in topological order of evaluation.
   */
   ...
   * 
   * @param {Array<Variable>} undeterminedVars
  ...
  var IncrementalQuickPlan = Class.create({
    /**
     * @param {CGraph} cgraph
     */
  ...
  ns.Solver = IncrementalQuickPlan;
}());
