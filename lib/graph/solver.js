...
(function () {
  ...
  var Solver = function (graph) {
    ...
    Object.keys(this.g.constraints).forEach(function (c) {
      this.constraints[c]
        = new Constraint(c, this.g.constraints[c], Strength.required);
    }, this);
  ...
  Solver.prototype.multiOutputPlanner = function multiOutputPlanner() {
    ...
    while (!this.cccToEnforce.isSatisfied() && this.freeVariableSet.length > 0) {
      ...
      if (vvvFree.isFree()) {
        ...
        LOG("MOP: free variable " + vvvFree.name + " attached to " + ccc.name);
  ...
  Solver.prototype.eliminateConstraint = function eliminateConstraint(ccc, mNext) {
  ...
  {
    LOG("CHP: attempt to enforce " + this.cccToEnforce.name);
           ...
           && this.retractableCnsQueue.length > 0)
    {
      ...
      this.strongestRetractedStrength
        = pickStronger(this.strongestRetractedStrength, ccc.strength);
    ...
    = function collectUnenforcedConstraints(undeterminedVars, cccNewlyEnforced)
  {
    ...
    mmNewlySelected.outputs.forEach(function (w) {
      this.collectDownstreamUnenforcedConstraints(this.g.variables[w]);
    }, this);
    ...
    = function collectDownstreamUnenforcedConstraints(vvv)
  {
    ...
    vvv.constraints.forEach(function (ccc) {
      if (!ccc.isSatisfied()) {
        if (!isStronger(ccc.strength, this.strongestRetractedStrength)) {
          this.unenforcedCnsQueue.push(ccc);
        }
        return;
      }
  ...
  Solver.prototype.solve = function solve(priority) {
    ...
    LOG("solve: new priority = " + JSON.stringify(this.priority));
    ...
    this.priorityPrev = priority.slice(0);
