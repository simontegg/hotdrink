...
(function () {
  ...
  var parse_then = function (sheet, continuation) {
    if (sheet === "") {
      return;
    }
    ...
    try {
      obj = hotdrink.parser.ModelParser.parse(sheet);
    } catch (e) {
      raid.error(e);
      return;
    }
    try {
      continuation(obj.cgraph, obj.methods);
    } catch (e) {
  ...
  var bind_dialog = function (sheet, inputs) {
    parse_then(sheet, bind_dialog_parsed, inputs);
  };
  ...
  var bind_dialog_parsed = function (cgraph, methods_txt, inputs) {
    ...
    hotdrink.controller.find.html.registerWidgets(factory);
    hotdrink.controller.bind.html.registerWidgets(factory);

    var tree = hotdrink.controller.find.html.findTree(model.variables);
